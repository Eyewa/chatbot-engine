...
from ..utils.response_formatter import (RESPONSE_TYPES,
...
class ChatService:
    ...
    def _extract_sql_results_count(
        self, debug_info: Optional[Dict[str, Any]]
    ) -> Optional[int]:
    ...
    def get_chat_history(self, conversation_id: Optional[str]) -> List[str]:
        ...
        try:
            ...
            if history and "messages" in history:
                ...
                logger.debug(
                    f"Loaded recent chat history for conversation_id={conversation_id}: {len(flat_history)} messages"
                )
                return flat_history
            else:
                return []
    ...
    def process_agent_result(self, agent_result: Any) -> Any:
        ...
        if isinstance(agent_result, dict):
            ...
            if all(isinstance(v, dict) for v in agent_result.values()):
                ...
                final_response_data = [
                    (
                        enforce_response_schema(item, RESPONSE_TYPES)
                ...
                final_response_data = enforce_response_schema(
                    agent_result, RESPONSE_TYPES
                )
            ...
            if len(processed_responses) > 1:
                ...
                final_response_data = (
                    processed_responses[0] if processed_responses else agent_result
                )
            ...
            if isinstance(agent_result, str):
                ...
                try:
                    ...
                    content_keywords = routing_config.get(
                        "response_type_detection", {}
                    ).get("content_keywords", {})
                except Exception as e:
                    logger.warning(
                        f"Could not load response type detection config: {e}"
                    )
                    content_keywords = {}
                ...
                for response_type, keywords in content_keywords.items():
                    if any(keyword in agent_result_lower for keyword in keywords):
                        response_type = (
                            routing_config.get("response_type_detection", {})
                            .get("type_mapping", {})
        ...
        if not isinstance(final_response_data, dict) and not (
            isinstance(final_response_data, list)
            and all(
                isinstance(item, dict) and "type" in item
                for item in final_response_data
            )
        ):
            final_response_data = generate_llm_message(
                agent_result,
    ...
    async def chat(
        ...
        user_message_id = self._chat_logger.start_conversation_message(
        ...
        try:
            ...
            conversation_message = await self._generate_conversation_message(
                user_input, final_response, conversation_id, user_message_id
            )
            ...
            logger.info(
                f"Chat completed with enhanced logging - Conversation: {conversation_id}, Duration: {conversation_duration_ms:.2f}ms"
            )
            ...
            logger.error(
                f"Chat failed with enhanced logging - Conversation: {conversation_id}, Error: {e}",
                exc_info=True,
            )
            return {
    ...
    async def _generate_conversation_message(
        self, user_input, final_response, conversation_id=None, message_id=None
    ):
        """
        ...
        try:
            ...
            metadata = {
                "conversation_id": conversation_id
                or os.environ.get("CONVERSATION_ID", "test-conv-id"),
            ...
            if hasattr(response, "content"):
                ...
                return (
                    response.get("message") or response.get("content") or str(response)
                )
