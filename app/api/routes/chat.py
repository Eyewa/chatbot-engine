...
@router.post(
    "/", response_model=ChatbotResponse, responses={500: {"model": ErrorResponse}}
)
async def chat_endpoint(
    request: ChatbotRequest, chat_service: ChatService = Depends(get_chat_service)
) -> ChatbotResponse:
    """
    ...
    try:
        ...
        if (
            ...
            and "conversation_message" in response_data
            and "output" in response_data
        ):
        ...
        response_obj = ChatbotResponse(
            conversation_message=conversation_message, output=output
        )
