...
from agent.core.utils import extract_last_n
from tools.sql_toolkit_factory import get_common_sql_tools, get_live_sql_tools
...
try:
    ...
    from langchain_core.runnables import RunnableBranch  # type: ignore
    ...
    class AgentExecutor:  # type: ignore
        def __init__(self, agent: Any = None, tools: Any = None, verbose: bool = False):
            self.agent = agent
...
def _extract_customer_id(query: str) -> Optional[str]:
    """Extract customer ID from a query string, if present."""
    match = re.search(r"customer\s+(\d+)", query, re.IGNORECASE)
    return match.group(1) if match else None


def _classify_query(query: str, classifier_chain: Any) -> str:
    """Classify the intent of a query using config keywords and a classifier chain."""
    q = query.lower()
...
def load_join_examples(path: str = "config/join_examples.json") -> list:
    """Load join examples from a JSON file."""
...
def find_relevant_examples(
    ...
    """Find relevant join examples using semantic search or tag-based matching."""
    if user_query:
        try:
            query_emb = SentenceTransformer("all-MiniLM-L6-v2").encode(
                user_query, convert_to_tensor=True
            )
            example_texts = [ex["description"] + " " + ex["example"] for ex in examples]
            example_embs = SentenceTransformer("all-MiniLM-L6-v2").encode(
                example_texts, convert_to_tensor=True
            )
...
def _build_agent(
    ...
    """Build an agent for a given database key and allowed tables."""
    if not LANGCHAIN_AVAILABLE:
        def dummy_agent(input_dict: dict) -> dict:
    ...
    relevant_examples = find_relevant_examples(
        allowed_tables, join_examples, user_query=user_query
    )
    ...
    logging.info(
        f"[AGENT] System prompt to LLM (JSON enforcement={'ALWAYS respond with a valid JSON object' in system_prompt}):\n{system_prompt}"
    )
...
def _create_live_agent() -> Any:
    """Create an agent for the 'live' database."""
    ...
    if not allowed:
        logging.error(
            "[AGENT] No allowed tables found for eyewa_live in config! LLM will hallucinate."
        )
...
def _create_common_agent():
    ...
    builder = PromptBuilder()
    system_prompt = builder.build_system_prompt(
        db="eyewa_common", allowed_tables=allowed, extra_examples=None
    )
    llm = ChatOpenAI(model="gpt-4o", temperature=0, max_retries=3)
    prompt = ChatPromptTemplate.from_messages(
        [
...
def apply_business_rules(combined_responses, user_query, response_types, schema):
    ...
    try:
        with open(
            os.path.join("config", "query_routing.yaml"), "r", encoding="utf-8"
        ) as f:
            routing_config = yaml.safe_load(f)
            rules = routing_config.get("business_rules", {}).get(
                "always_include_summary", []
            )
    except Exception:
        rules = []

    # Debug: log business rules
    logging.info(f"[DEBUG] business_rules: {rules}")
    logging.info(f"[DEBUG] user_query: {user_query}")
    ...
    for rule in rules:
        ...
        if condition == "customer_id_in_query":
            ...
            logging.info(f"[DEBUG] extracted customer_id: {customer_id}")
            already_present = any(
            ...
            if customer_id and not already_present:
                ...
                if customer_fields:
                    ...
                    customer_table = None
                    live_tables = schema
                    logging.info(f"[DEBUG] live_tables keys: {list(live_tables.keys())}")
                    for table_name, table_info in live_tables.items():
                        if (
                            "customer" in table_name.lower()
                            and "entity" in table_name.lower()
                        ):
                    ...
                    if rows:
                        ...
                        summary = build_summary(
                            rule_type, customer_data, schema, response_types
                        )
                        summary["customer_id"] = str(customer_id)
                        logging.info(f"[DEBUG] Appending customer_summary: {summary}")
...
def _combine_responses(resp_live, resp_common, user_query=None):
    ...
    def process_response(resp):
        if isinstance(resp, dict) and "type" in resp:
            ...
            return [
                build_summary(item["type"], item, schema, response_types)
                for item in resp
                if isinstance(item, dict)
                and "type" in item
                and item["type"] in response_types
            ]
        return None
    ...
    if user_query is not None:
        combined_responses = apply_business_rules(
            combined_responses, user_query, response_types, schema
        )
    ...
    if not combined_responses:
        ...
        try:
            with open(config_path, "r") as f:
                routing_config = yaml.safe_load(f)
            merge_strategies = routing_config.get("response_combination", {}).get(
                "merge_strategies", {}
            )
        except Exception:
            merge_strategies = {}
        return {
            "type": merge_strategies.get("default", "text_response"),
    ...
    if db == "common":
        ...
        for keyword in common_keywords:
            if re.search(rf"\b{re.escape(keyword)}\b", user_query, re.I):
                ...
                template = prompt_templates.get("common", {}).get(
                    keyword, f"show {keyword} information for customer {{customer_id}}"
                )
                result = template.format(customer_id=customer_id)

                logging.info(
                    f"[extract_focused_prompt] common match: '{keyword}' -> '{result}'"
                )
                return result

        logging.info("[extract_focused_prompt] common: no match found")
        return None
    else:
            ...
            if re.search(rf"\b{re.escape(keyword)}\b", user_query, re.I):
                if keyword == "order":
                    ...
                    limit_match = re.search(
                        r"(last|recent)\s+(\d+|one|two|three|four|five|six|seven|eight|nine|ten)",
                        user_query,
                        re.I,
                    )
                    limit_str = ""
                    ...
                    # Use config-driven template
                    template = prompt_templates.get("live", {}).get(
                        keyword,
                        "show {limit}orders{fields} for customer {customer_id}",
                    )
                    result = template.format(
                        limit=limit_str, fields=fields_str, customer_id=customer_id
                    )
                else:
                    # Use config-driven template for other keywords
                    template = prompt_templates.get("live", {}).get(
                        keyword,
                        f"show {keyword} information for customer {{customer_id}}",
                    )
                    result = template.format(customer_id=customer_id)

                logging.info(
                    f"[extract_focused_prompt] live match: '{keyword}' -> '{result}'"
                )
                return result

        logging.info("[extract_focused_prompt] live: no match found")
        return None
...
def inject_limit_phrase(prompt, limit):
    ...
    if not re.search(
        r"(last|recent)\s+(\d+|one|two|three|four|five|six|seven|eight|nine|ten)\s+(order|orders|record|records)",
        prompt,
        re.IGNORECASE,
    ):
        ...
        if "order" in prompt:
            ...
            result = (
                f"last {limit} orders {prompt[prompt.find('of'):].strip()}"
                if "of" in prompt
                else f"last {limit} orders {prompt}"
            )
        else:
...
def _handle_both(input_dict):
    ...
    if not live_prompt or not common_prompt:
        ...
        if live_prompt:
            return (
                live_agent.invoke(input_dict)
                if live_agent
                else {"type": "text_response", "message": "Live agent not available."}
            )
        if common_prompt:
            return (
                common_agent.invoke(input_dict)
                if common_agent
                else {"type": "text_response", "message": "Common agent not available."}
            )
        # If neither, something is wrong. Let the user know.
        return {
            "type": "text_response",
            "message": "Could not determine how to handle your request.",
        }
    ...
    try:
        ...
        return {
            "type": "text_response",
            "message": "There was an error fetching data from both sources.",
        }
            ...
            def invoke(self, input_dict):
                ...
                if intent == "live":
                    if live_agent is not None:
                        ...
                        return {
                            "type": "text_response",
                            "message": "Live agent is not available.",
                        }
                if intent == "common":
                    if common_agent is not None:
                        ...
                        return {
                            "type": "text_response",
                            "message": "Common agent is not available.",
                        }
...
def build_summary(summary_type, data, schema, response_types):
    ...
    for field in fields:
        ...
        summary[field] = value
    logging.info(f"[DEBUG] build_summary output for {summary_type}: {summary}")
    return summary
